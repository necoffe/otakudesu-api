...
import { downloadType, episodeType, genreType, mirrorStreamType, urlType } from "../types/detail_anime";
...
const detailEps = (detailEpsScrape: string) => {
    ...
    const downloadUrlsParse = $('.download ul li').toString().split('</li>');
    ...
    return {
        ...
        mirror_streams: parseMirrorStreams($),
        download_urls: downloadUrls
    };
};

const parseMirrorStreams = ($: ReturnType<typeof load>): mirrorStreamType[] => {
    const mirrors: mirrorStreamType[] = [];
    const qualities = ['m360p', 'm480p', 'm720p'];
    const qualityLabels: Record<string, string> = { m360p: '360p', m480p: '480p', m720p: '720p' };

    qualities.forEach(q => {
        $(`.mirrorstream ul.${q} li a`).each(function () {
            const dataContent = $(this).attr('data-content');
            const provider = $(this).text().trim();
            if (dataContent && provider) {
                mirrors.push({
...
export { detailAnime, detailEps, parseMirrorStreams };
